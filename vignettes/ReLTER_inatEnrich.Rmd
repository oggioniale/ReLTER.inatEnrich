...
`add_iucn_to_occ()` queries iNaturalist for each unique taxon and appends
a nested `status_IUCN` list-column. Each element is a tibble with one row
per geographic scope (global, regional), containing the IUCN status code,
authority, scope name, and a direct link to the IUCN Red List assessment.
...
`add_nativeness_to_occ()` queries iNaturalist for the establishment means
...
`add_eunis_legal_to_occ()` queries the EUNIS species database and appends
`directive` and `Annex` columns indicating coverage under the EU Habitats
...
```{r screenshot, echo = FALSE, eval = TRUE, out.width = "100%", fig.cap = "Interactive Leaflet map produced by create_leaflet_occ_map() for the Montagna di Torricchio eLTER-RI site (https://deims.org/6b62feb2-61bf-47e1-b97f-0e909c408db8)."}
htmltools::tags$iframe(
  src    = "figures/map_example.html",
  width  = "100%",
  height = "500px",
  style  = "border:none;"
)
